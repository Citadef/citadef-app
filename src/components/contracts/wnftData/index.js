...
import { MINTING_CONTRACT_ABI } from "components/contracts/ABIs/MintingContractABI.js"

import { fetchCID as ipfsFetchCID } from  'components/ipfsUtils';

import { getRpcUrl, getNetChainIdHex } from "components/contracts/chainUtils.js";
...
const fetchFishes = async (props) => {
    try {
        ...
        const provider = (  window.ethereum 
                            && chainId === getNetChainIdHex(process.env.REACT_APP_CHAIN) 
                            && new ethers.providers.Web3Provider(window.ethereum )
                         ) ||
                         new ethers.providers.JsonRpcProvider(getRpcUrl(process.env.REACT_APP_CHAIN));
