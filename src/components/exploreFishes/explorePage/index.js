...
import ExploreCard from "components/exploreFishes/exploreCard"
...
function ExplorePage(props) {
    ...
    for (let i=0; i<= Math.floor((fishes_keys.length-1)/FISH_IN_ROW); i++) {
        ...
        let big_fish_element = 
                ...
                <ExploreCard nick={big_fish.nick} seed={big_fish.seed} posts={big_fish.posts} col="col-md-12" size={big_fish_size} key={big_fish.seed} direction={direction} />
                </div>;
        ...
        if (i < Math.floor((fishes_keys.length-1)/FISH_IN_ROW)) {
            for (let j=1; j<5; j++) {
                ...
                small_fishes.push(<ExploreCard nick={small_fish.nick} seed={small_fish.seed} key={small_fish.seed} posts={small_fish.posts} col="col-md-6" size={small_fish_size} direction={direction}/>);
            }
