...
class GroovyFish {
    ...
    draw(svg, size, points, frame) {
        ...
        let rectangle_colorfill = this.getColor();
        let rectangle_colorstroke = this.getBrightColor();

        if (frame == "rectangle") 
            svg = this.rectangle(0.05*size, 0.05*size, 0.9*size, 0.9*size, svg, rectangle_colorfill, rectangle_colorstroke);
    ...
    rectangle(x, y, width, height, svg, colorfill, colorstroke) {
